...
/* the project.                                                               */
/*----------------------------------------------------------------------------*/
package com._604robotics.robot2015.utils;
 ...
 * PIDOutput
 */
public class AntiWindupPIDController implements LiveWindowSendable {

    public static final double kDefaultPeriod = .05;
//    private static int instances = 0;
    private double m_P;			// factor for "proportional" control
    private double m_I;			// factor for "integral" control
    private double m_C;			// factor for "integral" bound
    private double m_A;			// factor for "integral" alpha
    ...
    private AntiWindupPIDController.Tolerance m_tolerance;	//the tolerance object used to check if on target
    ...
    public class NullTolerance implements Tolerance {
    	...
    	public boolean onTarget() {
    		throw new RuntimeException("No tolerance value set when using AntiWindupPIDController.onTarget()");
    	}
    }

    private class PIDTask extends TimerTask {

        private AntiWindupPIDController m_controller;

        public PIDTask(AntiWindupPIDController controller) {
            if (controller == null) {
                throw new NullPointerException("Given AntiWindupPIDController was null");
            }
     ...
     * @param Kc the integral bound
     * @param Ka the integral alpha coefficient
     ...
     */
    public AntiWindupPIDController(double Kp, double Ki, double Kc, double Ka, double Kd, double Kf,
        ...
        m_controlLoop.schedule(new AntiWindupPIDController.PIDTask(this), 0L, (long) (m_period * 1000));

//        instances++;
//        UsageReporting.report(UsageReporting.kResourceType_PIDController, instances);
	m_tolerance = new AntiWindupPIDController.NullTolerance();
    }
     ...
     * @param period 
     */
    public AntiWindupPIDController(double Kp, double Ki, double Kc, double Ka, double Kd,
            PIDSource source, PIDOutput output,
            double period) {
        this(Kp, Ki, Kc, Ka, Kd, 0.0, source, output, period);
    }
    ...
    public AntiWindupPIDController(double Kp, double Ki, double Kc, double Ka, double Kd,
            PIDSource source, PIDOutput output) {
        this(Kp, Ki, Kc, Ka, Kd, source, output, kDefaultPeriod);
    }
    ...
    public AntiWindupPIDController(double Kp, double Ki, double Kc, double Ka, double Kd, double Kf,
            PIDSource source, PIDOutput output) {
        this(Kp, Ki, Kc, Ka, Kd, Kf, source, output, kDefaultPeriod);
    }
     ...
     * @param p Proportional coefficient
     * @param i Integral coefficient
     * @param c Integral bound
     * @param a Integral alpha coefficient
     * @param d Differential coefficient
     */
    public synchronized void setPID(double p, double i, double c, double a, double d) {
        m_P = p;
        m_I = i;
        m_C = c;
        m_A = a;
     ...
     * @param f Feed forward coefficient
     */
    public synchronized void setPID(double p, double i, double c, double a, double d, double f) {
        m_P = p;
        m_I = i;
        m_C = c;
        m_A = a;
    ...
    /**
     * Get the Integral alpha coefficient
     * @return integral alpha coefficient
     */
    public double getA() {
        return m_A;
    }
    ...
    /**
     * Set the setpoint for the AntiWindupPIDController
    ...
    /**
     * Returns the current setpoint of the AntiWindupPIDController
     ...
     * @deprecated Use setTolerance(Tolerance), i.e. setTolerance(new AntiWindupPIDController.PercentageTolerance(15))
     */
    public synchronized void setTolerance(double percent) {
        m_tolerance = new AntiWindupPIDController.PercentageTolerance(percent);
    }
     ...
     * creating the type of tolerance that you want to use: setTolerance(new AntiWindupPIDController.AbsoluteTolerance(0.1))
     * @param tolerance a tolerance object of the right type, e.g. PercentTolerance
     * or AbsoluteTolerance
     */
    private void setTolerance(AntiWindupPIDController.Tolerance tolerance) {
	...
	m_tolerance = new AntiWindupPIDController.AbsoluteTolerance(absvalue);
    }
	...
	m_tolerance = new AntiWindupPIDController.PercentageTolerance(percentage);
    }
    ...
    /**
     * Begin running the AntiWindupPIDController
     */
    ...
    /**
     * Stop running the AntiWindupPIDController, this sets the output to zero before stopping.
     */
    ...
    /**
     * Return true if AntiWindupPIDController is enabled.
     */
